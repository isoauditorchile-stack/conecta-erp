...
CREATE TABLE IF NOT EXISTS `iso27001_policies` (
  ...
  `policy_type` enum('politica','procedimiento','directriz','estandar') DEFAULT 'politica',
  ...
  `related_procedures` text DEFAULT NULL,
  `distribution_list` text DEFAULT NULL,
  `revision_history` text DEFAULT NULL,
  `compliance_requirements` text DEFAULT NULL,
  `training_required` enum('no','si') DEFAULT 'no',
  `acknowledgment_required` enum('no','si') DEFAULT 'no',
